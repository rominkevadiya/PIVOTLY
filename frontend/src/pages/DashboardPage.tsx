      ...
      <div className="space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 animate-fade-in-up">
          <div>
            <div className="flex items-center gap-3">
              <p className="text-sm font-bold uppercase tracking-widest text-moss text-gradient">
                Venture Hub
              </p>
              {stats && (
                <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-moss/10 text-moss border border-moss/20">
                  {stats.analyses_remaining_today} analyses left today
                </span>
              )}
            </div>
            <h1 className="text-4xl font-extrabold tracking-tight text-ink mt-2">
              Your Analysis History
            </h1>
            <p className="text-[15px] font-medium text-ink/70 mt-2">
              Compare your validation reports and strategic recommendations.
              {stats && stats.total_analyses > 0 && ` You have run ${stats.total_analyses} total analyses.`}
            </p>
          </div>
          <Link
            to="/analyze"
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            <div className="overflow-hidden rounded-2xl glass-panel animate-fade-in-up" style={{ animationDelay: '100ms' }}>
              <ul className="divide-y divide-white/50">
                {reports.map((report) => {
                  ...
                  return (
                    <li
                      key={report.id}
                      className="group relative hover:bg-white/60 transition duration-300 ease-in-out"
                    >
                      <Link to={`/reports/${report.id}`} className="block p-7">
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                          <div className="space-y-1.5 flex-1">
                            <div className="flex items-center gap-3">
                              <span className="text-xs text-ink/50 font-bold tracking-wider uppercase">
                                {dateStr}
                              </span>
                ...
                {reports.length === 0 && page > 1 && (
                  <li className="p-7 text-center text-ink/60">
            ...
            <div className="flex justify-between items-center px-4 animate-fade-in-up" style={{ animationDelay: '150ms' }}>
              <button
                onClick={() => setPage(p => Math.max(1, p - 1))}
                disabled={page === 1}
                className="px-4 py-2 text-sm font-semibold rounded-lg bg-white/50 text-ink/70 hover:bg-white hover:text-ink disabled:opacity-50 disabled:cursor-not-allowed transition"
              >
                Previous
              </button>
              <span className="text-sm font-medium text-ink/60">Page {page}</span>
              <button
